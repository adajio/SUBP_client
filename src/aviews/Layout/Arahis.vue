<template>
  <Toolbar style="height:var(--toolbar-height);padding:0px;border-radius: 0;background-color: var(--primary-color);"
    ...
    <template #start>
      <Button icon="mdi mdi-dots-grid" text rounded
        ...
        @click="toggle"></Button>
      <!-- <img src="/img/arahis/logo/logo.png" style="height:24px;"> -->
      <h3 style="color:white">АРАХИС</h3>
  ...
  <OverlayPanel ref="op" :pt="{ root: { style: 'padding:0px' } }">
    <ArahisPanel></ArahisPanel>
  </OverlayPanel>
  <ArahisSearch v-model:visible="searchVisible" v-model:searchText="searchText"></ArahisSearch>
  <slot></slot>
</template>
<script setup>
import { ref } from 'vue';
import { useTitle } from '@vueuse/core';
import ArahisPanel from './Arahis/Panel.vue';
import ArahisSearch from './Arahis/Search.vue';
...
// import ArahisSidebar from './ArahisSidebar.vue';
...
const userMenuItems = ref([
  { label: 'Настройки', icon: 'mdi mdi-cogs', to: '/arahis/admin' },
  { label: 'Язык', icon: 'mdi mdi-earth', to: '/arahis/admin' },
  { separator: true },
  { label: 'Выход', icon: 'mdi mdi-logout', to: '/logout' },
  { separator: true },
  { label: 'Помощь', icon: 'mdi mdi-help', to: '/arahis/help' },
  { label: 'О программе', icon: 'mdi mdi-about', to: '/arahis/about' }
]);
</script>
<style scoped>
.hover-gray:hover {
